...
def SimpleReflexAgentProgram(rules):
    ...
    def program(percept):
        ...
        state = percept
        rule, action = rule_match(state, rules)
        return action
...
def ModelBasedReflexAgentProgram(rules, update_state, transition_model, sensor_model):
    ...
    def program(percept):
        ...
        action = rule_match(program.state, rules)
        return action
...
class TrivialVacuumEnvironment(Environment):
    ...
    def execute_action(self, agent, action):
        ...
        print(f"\tAgent decides to {action} at {start_location}; new location is {self.percept(agent)[0]}")
